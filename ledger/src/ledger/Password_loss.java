...
public class Password_loss {
	...
	public Password_loss(String mailServer){
		...
		JFrame frame = new JFrame("비밀번호 분실");
		...
		JButton submit = new JButton("password 찾기");
		panel.add(submit);
	...
	private void find_password(){
		try{
	        ...
	        if(!rs.next()){
	        	JOptionPane.showMessageDialog(null, "password를 찾을 수 없습니다","찾기 실패", JOptionPane.PLAIN_MESSAGE);
	        }
	        else{
	    		...
	    		JOptionPane.showMessageDialog(null, "이메일로 임시 비밀번호가 발송되었습니다","임시 비밀번호 발송", JOptionPane.PLAIN_MESSAGE);
